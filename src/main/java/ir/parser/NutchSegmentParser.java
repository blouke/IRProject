...
public class NutchSegmentParser {
	...
	public static void main(String[] args) {
		...
		try {
			...
			SequenceFile.Reader reader = new SequenceFile.Reader(conf, Reader.file(file));

			Text key = new Text();
			Content content = new Content();

			while (reader.next(key, content)) {
				String filename = key.toString().replaceFirst("http://", "").replaceAll("/", "").trim();
